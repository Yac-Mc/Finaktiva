...
namespace API_Rest.Repositories
{
    public class GenericRepository<T> : BaseRepository, IGenericRepository<T> where T : class
    {
        ...
        public async Task<int> InsertListAsync(IEnumerable<T> list)
        {
            ...
            using (var db = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    await WithConnection(async conn =>
                    {
                        inserted += await conn.ExecuteAsync(_commandText.Insert(_columnNames, _tableName), list);
                        db.Complete();
                    });
                }
                catch
